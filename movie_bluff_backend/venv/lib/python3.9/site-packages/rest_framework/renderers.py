...
class DocumentationRenderer(BaseRenderer):
    ...
    def get_context(self, data, request):
        return {
            ...
            "lang_htmls": [
                "rest_framework/docs/langs/%s.html" % language
                for language in self.languages
            ],
            "lang_intro_htmls": [
                "rest_framework/docs/langs/%s-intro.html" % language
                for language in self.languages
            ],
            "code_style": pygments_css(self.code_style),
            "request": request,
...
class CoreAPIJSONOpenAPIRenderer(_BaseOpenAPIRenderer):
    ...
    def __init__(self):
        assert (
            coreapi
        ), "Using CoreAPIJSONOpenAPIRenderer, but `coreapi` is not installed."

    def render(self, data, media_type=None, renderer_context=None):
        structure = self.get_structure(data)
        return json.dumps(structure, indent=4, ensure_ascii=self.ensure_ascii).encode(
            "utf-8"
        )
...
class OpenAPIRenderer(BaseRenderer):
    ...
    def render(self, data, media_type=None, renderer_context=None):
        ...
        Dumper.add_representer(
            datetime.timedelta, encoders.CustomScalar.represent_timedelta
        )
        return yaml.dump(
